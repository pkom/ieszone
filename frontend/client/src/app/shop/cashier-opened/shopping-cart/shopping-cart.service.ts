...
export class ShoppingCartService {
  ...
  read(newBarcode: string): Observable<Shopping> {
    ...
    if (
      !Number.isNaN(price) &&
      newBarcode.length <= ShoppingCartService.VARIOUS_LENGTH
    ) {
    ...
    return this.articleService
      ...
      .pipe(
        ...
        catchError(() => {
          return this.dialog
            .open(ArticleQuickCreationDialogComponent, {
  ...
  createTicketAndPrintReceipts(
    ...
    requestDataProtectionAct: boolean,
  ): Observable<void> {
    return this.httpService.post(EndPoints.TICKETS, ticketCreation).pipe(
      concatMap((ticket) => {
        ...
        receipts = iif(
          () => voucher > 0,
          merge(receipts, this.createVoucherAndPrint(voucher)),
          receipts,
        );
        receipts = iif(
          () => requestedInvoice,
          merge(receipts, this.createInvoiceAndPrint(ticket.id)),
          receipts,
        );
        receipts = iif(
          () => requestedGiftTicket,
          merge(receipts, this.createGiftTicketAndPrint(ticket.id)),
          receipts,
        );
        receipts = iif(
          () => requestDataProtectionAct,
          merge(receipts, this.createDataProtectionActAndPrint(ticket)),
          receipts,
        );
        return receipts;
      }), // ,switchMap(() => EMPTY)
  ...
  printTicket(ticketId: string): Observable<void> {
    return this.httpService
      .pdf()
      .get(EndPoints.TICKETS + '/' + ticketId + ShoppingCartService.RECEIPT);
  }
